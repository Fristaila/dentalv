    ...
    <ul class="vacancy-list">
      <?php foreach ($rows as $id => $row): ?>
        <li class="vacancy-list-item">
          <span
               class="vacancy-title"><?php print $view->result{$id}->node_title ?></span>
          <p class="vacancy-desc"><?php print $view->result{$id}->field_body[0]['raw']['value']; ?></p>
          <span class="vacancy-link">
        <a class="vacancyResponseBtn link link_bordered_green link_bold"
           href="javascript:void(0)">
          <span><?php print t('Will respond') ?></span><span
               class="hide-for-large"><?php print t('to the vacancy'); ?></span></a>
      </span>
        </li>
      <?php endforeach; ?>
    </ul>
