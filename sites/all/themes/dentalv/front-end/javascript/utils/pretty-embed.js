	...
	$.fn.prettyEmbed = function (options) {
		...
		elem.each(function() {
			...
			if ( thisCustomPreviewImage !== undefined && thisCustomPreviewImage !== '' ) {
				$(this).css({
					'backgroundImage': 'url("' + thisCustomPreviewImage + '")',
					'backgroundColor': '#000'
				});
			}
			else {
				...
				$(this).css({
					'backgroundImage': 'url("//img.youtube.com/vi/' + thisVideoID + '/' + previewSizePrefix + '.jpg")',
					'backgroundColor': '#000'
				});
			}
		...
		// const touch = 'ontouchstart' in window ? "touchstart":"click";
		...
		$('body').on("click", '.pretty-embed', function(e) {
			...
			console.log($(this));
		}).on("click", '.my-trigger', function(e) {
			var $this = $(this);
			console.log("click");
			e.preventDefault();
			clickEventRunner($this.parents('.player-wrapper').find('.player') );
			$this.fadeOut(200);
			$this.parents(".video-about").addClass("clicked");
		...
		function clickEventRunner(obj) {
			...
			obj.html('<iframe width="' + wrapperWidth + '" height="' + wrapperHeight + '" ' + 'muted' + 'style="border:none;" src="//www.youtube.com/embed/' + thisVideoID + '?autoplay=1' + playerOptions + '"></iframe>')
